...
wages_growth_file = os.path.join(data_dir, "wages_growth.csv")
rent_growth_file  = os.path.join(data_dir, "rent_growth.csv")
fips_info = {
...
combined.to_csv(os.path.join(data_dir, "wages_vs_rent.csv"), index=False)

#%%
# Build heatmap data: pivot gap to wide, map fips to state names, sort worst to best
target_years = [yr for yr in [1995, 2000, 2005, 2010, 2015, 2020, 2024] if yr in combined["Year"].values]
...
ax.set_title(
    "US State Affordability gap by Year\n"
...
ax2.set_title(
    "National Average Affordability Gap Over Time\n"
    "Above Zero = Rent Outpacing Wages  |  Below Zero = Wages Outpacing Rent",
    fontsize=14, fontweight="bold", pad=14
)
ax2.set_xlabel("Year")
ax2.set_ylabel("Percentage points vs. Baseline")
